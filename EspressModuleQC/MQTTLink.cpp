/*
MQTTLink.cpp QControl Connection MQTT for esp8266
...
void callbackMQTT(const MQTT::Publish& pub) {     //callback when we get a msg from subtopics....
	...
	if (pub.topic() == MQTTLink.mqSubTopic + "/SetTemp")  //setTemp
	{
		...
		if (tbSetTemp >= 1) avrGlobal.SetTemp(tbSetTemp);
	}
...
void MQTTLinkClass::PublishMQTT()
{
	...
	if (WiFi.status() == WL_CONNECTED) {
		if (mqttclient.connected()) {
			mqttclient.publish(mqPubTopic + "/SetTemp", avrGlobal.avrSetPoint);
			mqttclient.publish(mqPubTopic + "/PitTemp", avrGlobal.avrPitTemp);
			mqttclient.publish(mqPubTopic + "/Food1", avrGlobal.avrFood1);
			mqttclient.publish(mqPubTopic + "/Food2", avrGlobal.avrFood2);
			mqttclient.publish(mqPubTopic + "/Food3", avrGlobal.avrAmbient);
			mqttclient.publish(mqPubTopic + "/Fan", avrGlobal.avrFan);
			mqttclient.publish(mqPubTopic + "/FanAvg", avrGlobal.avrFanMovAvg);
			mqttclient.publish(mqPubTopic + "/LidOpenCount", avrGlobal.avrLidOpenCountdown);			
		} //if mqtt connected
	}  //if wifi connected		
}
